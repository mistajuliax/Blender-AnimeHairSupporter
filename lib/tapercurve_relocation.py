...
class ahs_tapercurve_relocation(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for ob in context.visible_objects:
            if ob.type != 'CURVE':
                continue
            if ob not in taper_or_bevel_objects:
                continue
            ...
            for o in context.blend_data.objects:
                ...
                if self.mode == 'TAPER' and o.data.taper_object == ob:
                    parent_ob = o
                elif self.mode == 'BEVEL' and o.data.bevel_object == ob:
                    parent_ob = o
                elif self.mode == 'BOTH' and (o.data.taper_object == ob or o.data.bevel_object == ob):
                    parent_ob = o
            if not parent_ob:
                continue
        ...
        for ob, parent_ob in target_zips:
            if not len(parent_ob.data.splines):
                continue

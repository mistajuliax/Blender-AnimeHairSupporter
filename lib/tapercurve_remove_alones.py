    ...
    def poll(cls, context):
        try:
            ...
            for ob in context.blend_data.objects:
                ...
                if re.search(r':(Taper|Bevel)$', name) and ob not in taper_and_bevel_objects:
                    break
            else:
                return False
        except:
        ...
        if self.mode != 'BEVEL':
            ...
            for ob in context.blend_data.objects:
                if ob.type != 'CURVE':
                    continue
                if ob.data.taper_object:
                    taper_objects.append(ob.data.taper_object)
            for ob in context.blend_data.objects:
                name = re.sub(r'\.\d{3,}$', "", ob.name)
        ...
        if self.mode != 'TAPER':
            ...
            for ob in context.blend_data.objects:
                if ob.type != 'CURVE':
                    continue
                if ob.data.bevel_object:
                    bevel_objects.append(ob.data.bevel_object)
            for ob in context.blend_data.objects:
                name = re.sub(r'\.\d{3,}$', "", ob.name)
